...
const tools = [
  ...
  },
  {
        ...
        description: "Directly update the daily budget for a campaign (in cents). Use execute_budget_change for user-approved changes.",
        inputSchema: {
                type: "object",
                properties: {
                          campaign_id: { type: "string" },
                          daily_budget: { type: "number", description: "New daily budget in cents. e.g. 5000 = $50.00" },
                },
  ...
  },
  {
        ...
        description: "Update the daily budget for a specific ad set (in cents)",
        inputSchema: {
                type: "object",
                properties: {
...
async function executeTool(name, args) {
    switch (name) {
      ...
      case "update_campaign_budget":
              return await metaAPI(`/${args.campaign_id}`, "POST", { daily_budget: args.daily_budget });
      case "update_adset_budget":
              return await metaAPI(`/${args.adset_id}`, "POST", { daily_budget: args.daily_budget });
      ...
      case "analyze_and_suggest_scaling": {
              ...
              const analysis = campaigns.map((c) => {
                        ...
                        let priority = "LOW";
                                                     ...
                                                     if (c.status === "PAUSED") {
                                                                 recommendation = "PAUSED"; action = "Review before reactivating";
      ...
      case "execute_bulk_budget_changes": {
              ...
              for (const c of args.changes) {
                        try {
                                    ...
                                    results.push({ campaign: c.campaign_name || c.campaign_id, budget: `$${(c.new_daily_budget_cents / 100).toFixed(2)}/day`, status: res.error ? "Failed" : "OK", error: res.error });
                        } catch (e) {
          ...
          try {
                  if (!ACCESS_TOKEN) {
                            return {
                                        jsonrpc: "2.0",
                                        id,
                                        result: { content: [{ type: "text", text: "Error: META_ACCESS_TOKEN environment variable is not set." }] },
                            };
                  }
                  ...
                  return {
                            jsonrpc: "2.0",
                            id,
                            result: { content: [{ type: "text", text: `Error executing tool: ${err.message}` }] },
                  };
          }
    }
...
// Messages endpoint - Claude POSTs MCP requests here
app.post("/messages", async (req, res) => {
    const sessionId = req.query.sessionId;
    const sseRes = sessions.get(sessionId);

           if (!sseRes) {
                 return res.status(400).json({ error: "Session not found. Please reconnect to /sse first." });
           }
...
// Direct POST fallback for non-SSE clients
...
// Health check
app.get("/health", (_, res) => res.json({ status: "healthy", uptime: process.uptime() }));

// --- Start ---
const PORT = process.env.PORT || 8080;
app.listen(PORT, "0.0.0.0", () => {
    console.log(`Meta Ads MCP Server v2.0 running on port ${PORT}`);
    console.log(`  Token: ${ACCESS_TOKEN ? "Set" : "MISSING - set META_ACCESS_TOKEN"}`);
    console.log(`  Account: ${AD_ACCOUNT_ID || "MISSING - set META_AD_ACCOUNT_ID"}`);
});
